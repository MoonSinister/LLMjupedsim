...
def identify_profile(record):
    ...
    history_hint = _build_history_hint(top_activities, top_categories, summary["top_areas"])
    intent_hint = _build_intent_hint(role, subtype, preferences)
    memory_hint = _shorten(record.train_routine_list[-1] if record.train_routine_list else "", 240)
...
def _build_history_hint(top_activities, top_categories, top_areas):
    parts = []
    if top_categories:
        parts.append("frequent categories: " + ", ".join(top_categories[:4]))
    if top_activities:
        parts.append("frequent activities: " + ", ".join(top_activities[:4]))
    area_names = [name for name, _ in top_areas[:4]]
    if area_names:
        parts.append("frequent areas: " + ", ".join(area_names))
    return "; ".join(parts)


def _build_intent_hint(role, subtype, preferences):
    pref_text = ", ".join(preferences[:4]) if preferences else "available semantic regions"
    return f"Likely indoor intent for {role}/{subtype}: choose from {pref_text}, then leave through a plausible exit."
...
def build_llmob_profile_sampler(data_root, dataset="2019", max_persons=0, seed=2026, cache_output=None):
    ...
    profiles = [{"schema_version": "1.0", **identify_profile(record)} for record in records]
    if cache_output:
        path = pathlib.Path(cache_output)
        path.parent.mkdir(parents=True, exist_ok=True)
        path.write_text(
            json.dumps({"schema_version": "1.0", "profiles": profiles}, ensure_ascii=False, indent=2),
            encoding="utf-8",
        )
...
def atc_partition(person_id, modulus=10):
    """Assign a stable person-level split without exposing identifiers across splits."""
    bucket = int(hashlib.sha256(str(person_id).encode("utf-8")).hexdigest()[:8], 16) % modulus
    return "evaluation" if bucket == 0 else "tuning" if bucket == 1 else "train"


def load_atc_tracks(raw_path, regions_path=None, max_persons=0, max_rows=0, min_points=300, partition="all"):
    ...
    for csv_path in iter_atc_csv_files(raw_path):
        with csv_path.open("r", encoding="utf-8", newline="") as handle:
            ...
            for row in rows:
                ...
                if partition != "all" and atc_partition(person_id) != partition:
                    continue
...
def identify_atc_profile(track):
    ...
    history_hint = _build_atc_history_hint(summary, visited_regions)
    return {
        ...
        "preferences": preferences,
        "history_hint": history_hint,
        "intent_hint": f"Likely {role} route inferred from observed speed, dwell ratio, and visited indoor regions.",
        "memory_hint": _format_atc_reference(track, visited_regions),
...
def _build_atc_history_hint(summary, visited_regions):
    region_text = ", ".join(visited_regions[:5]) if visited_regions else "unlabeled indoor path"
    return (
        f"ATC evidence: duration={summary['duration_seconds']}s, "
        f"path_length={summary['path_length_m']}m, mean_speed={summary['mean_speed_m_s']}m/s, "
        f"slow_ratio={summary['slow_ratio']}, regions={region_text}."
    )
...
def build_atc_profile_sampler(
    ...
    profiles = [{"schema_version": "1.0", **identify_atc_profile(track)} for track in tracks]
    if cache_output:
        path = pathlib.Path(cache_output)
        path.parent.mkdir(parents=True, exist_ok=True)
        path.write_text(
